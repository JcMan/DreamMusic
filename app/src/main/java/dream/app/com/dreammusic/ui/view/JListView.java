 ...
 *
 */
public class JListView extends LinearLayout implements OnTouchListener {
    ...
    private boolean loadOnce;  //否已加载过一次layout，这里onLayout中的初始化只需加载一次
    ...
    public JListView(Context context, AttributeSet attrs) {
        ...
        view_pro_text = header.findViewById(R.id.layout_pro_text);
        ...
        if (ableToPull) {
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    int distance = (int) ((yMove - yDown)/2.0);
...
//                Toast.makeText(mContext,"刷新完成",Toast.LENGTH_SHORT).show();
                view_pro_text.setVisibility(GONE);
            }
        });
    ...
    private void updateHeaderView() {
        if (lastStatus != currentStatus) {
            if (currentStatus == STATUS_PULL_TO_REFRESH){
                ...
                view_pro_text.setVisibility(VISIBLE);
    ...
    public void readyToRefresh(){
        headerLayoutParams.topMargin = (200 / 2) + hideHeaderHeight;
        header.setLayoutParams(headerLayoutParams);
        currentStatus =STATUS_RELEASE_TO_REFRESH;
        new RefreshingTask().execute();
    }
