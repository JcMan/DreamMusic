...
public class SingerImageActivity extends Activity implements View.OnClickListener{
    ...
    private void initPagerViews() {
        ...
        if(mUrlsList.size()!=0){
            for(int i=0;i<mUrlsList.size();i++){
                ...
                finalBitmap.display(iv, mUrlsList.get(i), bitmap_loading, bitmap_failure);
                mViewsList.add(iv);
            }
        }else{
            ImageView iv = new ImageView(this);
    ...
    public void getDateFromIntent() {
        ...
        try {
            ...
            String baseurl = root.getAttribute("baseurl");
            int num = Integer.parseInt(root.getAttribute("num"));
            NodeList list = root.getElementsByTagName("img");
            for (int i = 0; i < list.getLength(); i++) {
                Element e = (Element) list.item(i);
                String link = e.getAttribute("link");
                mUrlsList.add(baseurl+link);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
